//==============简单请求==============
	...
	function 原神(text, voice, volume, speed, pitch) {
		...
		speed = (speed || 50)* 0.02 ;
		var type = Timbre == '高品质' ? '':'v1/',
		format = Timbre == '高品质' ? 'wav':'mp3',
		url = 'https://genshinvoice.top/'+type+'api?speaker='+voice+'&format='+format+'&length=1&noise=0.2&noisew=0.9&sdp_ratio=0.8&text='+text
		朗读(url)
	}
...
//==============复杂请求==============
	...
	function 百度(text, voice, volume, speed, pitch) {
		...
		volume = Math.floor((volume || 50) * 0.15);
	...
	function 讯飞(text, voice, volume, speed, pitch, emotion) {
		加载('js/CryptoJS.min.js', function() {
			讯飞(text, voice, volume, speed, pitch, emotion)
		})
 ...
 //http://www.xylsok.com/api/ra
	...
	function 大声(text, voice, volume, speed, pitch) {
		...
		var speechConfig = 'X-Timestamp:' + new Date() + '\r\nContent-Type:application/json; charset=utf-8\r\nPath:speech.config\r\n\r\n{"context":{"synthesis":{"audio":{"metadataoptions":{"concatenateResult": "false","decompressOutputFiles": "false","sentenceBoundaryEnabled":"false","wordBoundaryEnabled":"false"},"outputFormat":"'+ format +'"}}}}'
		...
		ws.onmessage = function(evt) {
			if (evt.data instanceof Blob) {
				...
				function fn1(ab) {
					...
					var viewarray = view.map(function(i) {return +i});
					//bufferList.push(...viewarray);
					var v , n = viewarray.length;
					for(v = 0; v < n ; v++) {
						bufferList.push(viewarray[v])
					} 
				}
	...
	function 微硬(text, voice, volume, speed, pitch) {
		...
		function geturl() {
			...
			eval(fn)({
				...
				data: body,
				dataType: 'text',
				referer: 'https://www.text-to-speech.cn/js/console.js',
				error: geturl,
				success: function(res) {
	...
	function 马克(text, voice, volume, speed, pitch) {
		...
		function gettoken() {
			uuid = UUID();
			var reqUrl = 'https://ttsmaker.cn/get_captcha?uuid='+uuid
			验证码(reqUrl, fn)
